	...
	if (pipe(fd_sm) == -1)
  {
	...
	for(i = 0; i < number_of_slaves; i++)
	{
		...
		if(fork() == 0)
		{
			...
			close(fd_sm[READ]);  // Close reading end of second pipe
			int number_read;
			...
			write( fd_sm[WRITE], &number_of_primes, sizeof(int));
			printf("Number of primes read by process %d: %d\n", getpid(), number_of_primes);
			close(fd_sm[WRITE]);  // Close writing end of second pipe
			exit(0);
		}
	...
	int total_of_primes = 0;
	while(read(fd_sm[READ], &number_of_primes, sizeof(int)) != 0 ) //read from pipe
	{
		total_of_primes += number_of_primes;
	}
